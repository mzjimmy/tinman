import type { AppState, FrameworkAdvice, GoalCard, Project, Task } from './types'
import { completeTaskCriteria } from './progress'

let taskCounter = 142

function stubAdvice(): FrameworkAdvice {
  return {
    diagnosis: 'round 2 LLM not connected',
    next_step: '确认架构地图后再派发',
    entry_point: '.',
    shared_risk: '与相邻部位共用约定，由本任务确定',
    done_criteria: ['验收项全部满足', '不写入扫描根目录', '不 push / deploy / spend'],
  }
}
...
export function draftGoal(
  ...
  return {
    ...
    attachments: [],
    facts: {
      last_commit: part?.facts?.lastCommit ?? '',
    },
    framework_advice: {
      next_step: advice.next_step,
      entry_point: advice.entry_point,
      shared_risk: advice.shared_risk,
    },
    done_criteria: advice.done_criteria,
    assumptions: [
      'ASSUMED: 派发在 round 2 接入 agent CLI — 因为本轮只保留任务卡形状。反转方式：接上 worktree 派发。',
    ],
    workspace: {
