...
export function RobotSvg({
  ...
  return (
    ...
    >
      ...
      {renderEyes(bySlot.head, geo.head)}
      {showReady && <text x="178" y="24" className="ready-badge">READY</text>}
    </svg>
  )
}
...
function renderUnit(
  ...
  const status = !mapConfirmed
    ? 'unmapped'
    : isGhost
      ? 'unmapped'
      : dp!.status
  const progress = mapConfirmed && dp ? dp.displayProgress : 0
  const clipId = `${uid}-clip-${slot}`
  const fh = rect.h * (progress / 100)
  const fy = rect.y + rect.h - fh
  const tooltip = dp
    ? `${dp.part.label} ｜ ${SLOT_LABELS[slot]}\n进度 ${progress}%　${STATUS_LABELS[status]}${dp.shortLegScore > 0 ? `\n短腿分数 ${dp.shortLegScore.toFixed(1)}` : ''}`
    : `${SLOT_LABELS[slot]}\n未映射`

  if (isGhost || (!mapConfirmed && dp)) {
    return (
      <g
  ...
  if (head && bySlot.head) {
    s = (
      <>
        ...
        <circle
          className={`bulb${bySlot.head.status === 'done' ? ' on' : ''}`}
          cx="100"
          cy={head.y - 14}
          r="3.2"
        />
